...
uploaded_file = st.file_uploader("파일 업로드 (all responses 시트 필수)", type=['xlsx'])
...
if uploaded_file:
    try:
        ...
        for i, v in enumerate(chart_df['점수']):
        ...
        with c2:
            # 초대형 점수표
            html = f"""
            <div style='border:2px solid #4A90E2; padding:15px; border-radius:10px; background-color:#fff;'>
        ...
        if st.button("🚀 분석 실행"):
            with st.spinner("보안 연결 확인 및 AI 분석 중..."):
                all_text = ""
                ...
                try:
                    ...
                    response = model.generate_content(f"교육 전문가로서 아래 설문 답변을 분석해줘:\n{all_text}")
                    ...
                    st.error(f"AI 분석 오류 (보안 연결 확인 필요): {e}")
